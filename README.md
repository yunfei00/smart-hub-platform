...
- AI 助手：本地模型接入（OpenAI 兼容接口，默认 Ollama）+ 多轮会话（新建/切换/连续追问）+ 推荐入口“推荐 -> 用户点击 -> 页面跳转”。
- 统一配置：支持通过环境变量覆盖关键参数。
...
## Phase 16：AI 多轮对话（最小可用）

本阶段在既有 AI 助手基础上，新增会话与消息存储，支持 Web 端多轮连续追问。

### 已支持能力

- 会话管理：支持新建会话、查看会话列表、进入指定会话继续提问。
- 消息管理：按 `user/assistant/system` 角色记录消息；当前 MVP 主要写入 `user` 与 `assistant`。
- 上下文策略：调用本地模型时，默认仅带入当前会话最近 N 条消息（窗口方式）。
- 新会话标题：默认标题为“新会话”，首条用户消息提交后自动生成简短标题。
...
1. 打开 `/ai-assistant/`，点击“新建会话”，应看到左侧出现新会话。
2. 进入某会话连续发送两次问题，页面右侧应按时间顺序显示多条用户/助手消息。
3. 切换到另一个会话后再返回，历史消息应保持。
4. 通过 `python manage.py migrate` 完成数据库迁移后，可在后台数据库中看到 `Conversation` 与 `Message` 两张表。
